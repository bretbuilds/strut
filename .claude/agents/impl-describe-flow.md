...
- `.strut-pipeline/spec-refinement/spec.json` — use `what`, `criteria[]`, and `implementation_notes` to understand the scope and intent of the change. The criteria tell you which behaviors exist; the implementation notes tell you which files are involved.
- `.strut-pipeline/impact-scan.md` — human-readable evidence map from the scan. Use to understand the broader system context around the changed files.
...
`.strut-pipeline/impl-describe-flow.txt`
...
1. Run `rm -f .strut-pipeline/impl-describe-flow.txt`.
2. Read `.strut-pipeline/spec-refinement/spec.json` and `.strut-pipeline/impact-scan.md`. If `spec.json` is missing or malformed, stop without writing the output file. The orchestrator detects the missing file and handles the failure.
...
7. Write `.strut-pipeline/impl-describe-flow.txt` using the format above. Write the end-state description, not a changelog. Stop.
...
- Write only `.strut-pipeline/impl-describe-flow.txt`. No other writes.
